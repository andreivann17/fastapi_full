...
biomarkers. Authentication is enforced on sensitive operations using
...
import re
from ..models.biomarkers import biomarkers_model, edit_biomarker_by_code, get_biomarkers,get_biomarker_by_code_sql  # <-- NUEVO import
...
def get_unique_code(max_tries: int = 5) -> str:
    ...
    for _ in range(max_tries):
        ...
        if not biomarkers_model.code_exists(code):
            return code
...
async def insert_biomarker(
    ...
    """
    Create a new biomarker (multipart/form-data). Guarda email si se envía.
    """
    ...
    if img is not None:
        ...
        img_rel_path = f"/uploads/biomarkers/{filename}"

    # 5) Insert
    try:
        biomarkers_model.add_biomarker(
            code=code,
    ...
    # 6) Respuesta
    return {
        "message": "The biomarker information was created successfully",
        "code": code,
        "img": img_rel_path,
    }

@router.get("/by-id/{biomarker_id}", deprecated=True)
def get_biomarker_by_id_legacy(
    biomarker_id: int,
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> Dict[str, Any]:
    # Si aún necesitas el viejo flujo un tiempo:
    return get_biomarkers(biomarker_id)

@router.get("/{code}")
def get_biomarker_by_code(
    code: str,
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> Dict[str, Any]:
    """
    Retrieve a single biomarker along with lookup data, by biomarker CODE.
    ...
    data = get_biomarker_by_code_sql(code.upper())
    if not data:
...
def list_biomarkers(
    ...
    """Retrieve all biomarkers with lookup data."""
    return get_biomarkers(None)


class biomarkersCardsRequest(BaseModel):
    limit: int = 10
    search: Optional[str] = ""
    offset: int = 0
...
@router.post("/cards")
def get_biomarkers_cards(
    payload: biomarkersCardsRequest,
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> List[Dict[str, Any]]:
    """Return a paginated list of biomarker cards."""
    return biomarkers_model.get_biomarkers_cards(
...
async def update_biomarker(
    ...
    """
    Update biomarker identified by 'code'.
    ...
    current_img = biomarkers_model.get_current_img_by_code(code)  # <-- NUEVO helper en modelo
    ...
    if img is not None:
        ...
        new_img_path = f"/uploads/biomarkers/{filename}"

    payload["img"] = new_img_path
    payload["code"] = code  # asegurar

    try:
        edit_biomarker_by_code(payload, user_id)  # <-- UPDATE usando WHERE code = %s
    except Exception:
    ...
    return {"message": "biomarker updated successfully", "img": payload["img"]}
...
def delete_biomarker(
    ...
    if not user_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid user ID")
    biomarkers_model.delete_biomarker(code, user_id)
    return {"message": "biomarker successfully deleted", "code": code}

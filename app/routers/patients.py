...
patients. Authentication is enforced on sensitive operations using
...
import re
from ..models.patients import patients_model, edit_patient_by_code, get_patients,get_patient_by_code_sql  # <-- NUEVO import
...
def get_unique_code(max_tries: int = 5) -> str:
    ...
    for _ in range(max_tries):
        ...
        if not patients_model.code_exists(code):
            return code
...
async def insert_patient(
    ...
    """
    Create a new patient (multipart/form-data). Guarda email si se envía.
    """
    ...
    if img is not None:
        ...
        img_rel_path = f"/uploads/patients/{filename}"

    # 5) Insert
    try:
        patients_model.add_patient(
            code=code,
            first_name=firstName.strip(),
            last_name=lastName.strip(),
            email=(email or "").strip(),
            gender_id=idGender,
            blood_type_id=idBloodType,
            birth_date=bdate,
            city=city,
            state=state,
            country=country,
    ...
    # 6) Respuesta
    return {
        "message": "The patient information was created successfully",
        "code": code,
        "img": img_rel_path,
    }

@router.get("/by-id/{patient_id}", deprecated=True)
def get_patient_by_id_legacy(
    patient_id: int,
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> Dict[str, Any]:
    # Si aún necesitas el viejo flujo un tiempo:
    return get_patients(patient_id)
...
def get_patient_by_code(
    ...
    """
    Retrieve a single patient along with lookup data, by patient CODE.
    ...
    data = get_patient_by_code_sql(code.upper())
    if not data:
...
def list_patients(
    ...
    """Retrieve all patients with lookup data."""
    return patients_model.get_patients(None)


class PatientsCardsRequest(BaseModel):
    limit: int = 10
    search: Optional[str] = ""
    offset: int = 0
...
def get_patients_cards(
    payload: PatientsCardsRequest,
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> List[Dict[str, Any]]:
    """Return a paginated list of patient cards."""
    return patients_model.get_patients_cards(
        limit=payload.limit, search=payload.search or "", offset=payload.offset
    )
@router.get("/{code}/timeline")
def get_patient_timeline(
    code: str,
    startDate: str = Query(..., description="YYYY, YYYY-MM o YYYY-MM-DD"),
    endDate: str   = Query(..., description="YYYY, YYYY-MM o YYYY-MM-DD"),
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> Dict[str, Any]:
    """
    Devuelve una lista mensual con una imagen por mes (la detección más reciente).
    Si un mes no tiene detecciones, se incluye con img = null.
    """
    # Validar code (ajusta patrón a tu longitud real, p.ej. 12)
    if not re.fullmatch(r"[A-Z0-9]{8,24}", code.upper()):
        raise HTTPException(status_code=422, detail="Invalid code format")

    start_m = _parse_to_month_start(startDate)
    end_m   = _parse_to_month_start(endDate)

    # Rango SQL: [start_m, end_exclusive)
    start_sql = datetime.combine(start_m, datetime.min.time())
    end_excl  = datetime.combine((end_m + relativedelta(months=1)), datetime.min.time())

    # 1) Resolver id_patient por code
    conn = get_connection()
    try:
        with conn.cursor(dictionary=True) as cur:
            cur.execute("SELECT id_patient FROM patients WHERE code=%s LIMIT 1", (code.upper(),))
            row = cur.fetchone()
            if not row:
                raise HTTPException(404, detail="Patient not found")
            id_patient = row["id_patient"]

            # 2) Para cada mes del rango, elegir la detección más reciente del mes
            #    Usamos subconsulta para tener la MAX(datetime) por (año, mes), y luego traer su fila para obtener img.
            sql = """
            SELECT
            DATE_FORMAT(d.`datetime`, '%Y-%m-01') AS month_key,
            d.id_detection,
            d.img,
            d.`datetime`
            FROM detections d
            ...
            ON YEAR(d.`datetime`)  = x.y
            AND MONTH(d.`datetime`) = x.m
            AND d.`datetime`        = x.max_dt
            WHERE d.id_patient = %s
            ORDER BY d.`datetime` DESC
            """
            cur.execute(sql, (id_patient, start_sql, end_excl, id_patient))
            rows = cur.fetchall()
    ...
    print(rows)
    for r in rows:
        by_month[r["month_key"]] = {
            "id_detection": r["id_detection"],
            "img": r["img"],
            "datetime": r["datetime"].isoformat() if isinstance(r["datetime"], datetime) else r["datetime"],
            "has_data": True,
        }

    # 4) Construir línea de tiempo con huecos (meses sin datos)
    months = _month_range_inclusive(start_m, end_m)
    timeline = []
    for m in months:
        key = m.strftime("%Y-%m-01")
        item = by_month.get(key)
        if item is None:
            timeline.append({
    ...
    return {
        ...
        "start_month": start_m.strftime("%Y-%m"),
        "end_month": end_m.strftime("%Y-%m"),
        "count_months": len(timeline),
        "timeline": timeline,
    }
...
async def update_patient(
    ...
    """
    Update patient identified by 'code'.
    ...
    current_img = patients_model.get_current_img_by_code(code)  # <-- NUEVO helper en modelo
    ...
    if img is not None:
        ...
        new_img_path = f"/uploads/patients/{filename}"

    payload["img"] = new_img_path
    payload["code"] = code  # asegurar

    try:
        edit_patient_by_code(payload, user_id)  # <-- UPDATE usando WHERE code = %s
    except Exception:
    ...
    return {"message": "Patient updated successfully", "img": payload["img"]}
@router.patch("/delete/{patient_id}", status_code=200)
def delete_patient(
    patient_id: int,
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> Dict[str, Any]:
    ...
    if not user_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid user ID")
    patients_model.delete_patient(patient_id, user_id)
    return {"message": "Patient successfully deleted", "id_patient": patient_id}

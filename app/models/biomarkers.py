...
class biomarkersModel:
    def code_exists(self, code: str) -> bool:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT 1 FROM biomarkers WHERE code=%s LIMIT 1", (code,))
    ...
    def get_current_img_by_code(self, code: str) -> str:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT img FROM biomarkers WHERE code=%s", (code,))
                row = cur.fetchone()
                if not row:
                    raise ValueError("biomarker not found")
                (img,) = row
                return img or ""
        finally:
            conn.close()
    ...
    def add_biomarker(
        ...
        *,
        ...
        try:
            ...
            conn.close()
    def delete_biomarker(self, code: str, user_id: int) -> None:
        conn = get_connection()
        try:
            with conn.cursor() as cur:
                cur.execute(
    ...
    def get_biomarkers_cards(
        ...
        try:
            with conn.cursor(dictionary=True) as cur:
                base = (
                    "SELECT id_biomarker AS id, code, CONCAT(first_name,' ',last_name) AS name, email "
                    "FROM biomarkers WHERE active = 1"
                )
                params: List[Any] = []
                if search:
                ...
                base += " ORDER BY id_biomarker DESC LIMIT %s OFFSET %s"
...
biomarkers_model = biomarkersModel()
...
def get_biomarkers(biomarker_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        with conn.cursor(dictionary=True) as cur:
            if biomarker_id:
                cur.execute(
                    ...
                    FROM biomarkers p
                    WHERE p.active = 1 AND p.id_biomarker = %s
                    """,
                    (biomarker_id,),
                )
            else:
        ...
        conn.close()
def get_biomarker_by_code_sql(code: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        conn.close()
def edit_biomarker_by_code(data: Dict[str, Any], user_id: int) -> None:
    """
